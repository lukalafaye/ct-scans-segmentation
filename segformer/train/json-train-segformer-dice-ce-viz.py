...
class CombinedLoss(torch.nn.Module):
    ...
    def forward(self, inputs, targets, expected_labels):
        ...
        # Cross Entropy Loss (using full mask)
        target_indices = torch.argmax(targets, dim=1)  # shape: (B, 256,256)
        ce = self.ce_loss(inputs, target_indices)

        # Prepare for Dice Loss: remove background channel (assumed channel 0)
        inputs_dice = inputs[:, 1:, :, :]  # shape: (B, MAX_ITEMS-1, 256,256)
        targets_dice = targets[:, 1:, :, :]  # shape: (B, MAX_ITEMS-1, 256,256)
        B, C_dice, H, W = inputs_dice.shape
        inputs_flat = inputs_dice.view(B, C_dice, -1)   # shape: (B, C_dice, H*W)
        targets_flat = targets_dice.view(B, C_dice, -1)

        # Compute dice per sample and per channel
        intersection = (inputs_flat * targets_flat).sum(dim=2)  # (B, C_dice)
        union = inputs_flat.sum(dim=2) + targets_flat.sum(dim=2)  # (B, C_dice)
        dice = (2. * intersection + self.smooth) / (union + self.smooth)  # (B, C_dice)

        # Create a mask to determine which channels to include in the dice loss
        # For each sample i and channel j (which corresponds to organ label j+1):
        #   If organ (j+1) is expected (per JSON) but target has zero pixels, exclude this channel.
        mask = torch.ones((B, C_dice), device=inputs.device)
        target_sums = targets_flat.sum(dim=2)  # shape: (B, C_dice)
        for i in range(B):
            for j in range(C_dice):
                organ_label = j + 1  # because channels 0...C_dice-1 correspond to labels 1...MAX_ITEMS-1
                if (organ_label in expected_labels[i]) and (target_sums[i, j] == 0):
                    mask[i, j] = 0.0

        # Compute dice loss only over channels that are not masked out.
        dice_losses = []
        for i in range(B):
            if mask[i].sum() > 0:
        ...
        combined_loss = 0.1 * ce + 0.9 * dice_loss
